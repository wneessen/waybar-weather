...
func TestNewGeolocationGeoAPIProvider_locate(t *testing.T) {
	t.Run("locate fails on invalid coordinate parsing", func(t *testing.T) {
		tests := []struct {
			...
			{name: "zip", file: "../../../../testdata/geoip.json", want: geobus.AccuracyZip},
			{name: "city", file: "../../../../testdata/geoip_nozip.json", want: geobus.AccuracyCity},
			{name: "region", file: "../../../../testdata/geoip_nocity.json", want: geobus.AccuracyRegion},
			{name: "country", file: "../../../../testdata/geoip_noregion.json", want: geobus.AccuracyCountry},
			{name: "unknown", file: "../../../../testdata/geoip_nocountry.json", want: geobus.AccuracyUnknown},
		}
...
func TestGeolocationGeoAPIProvider_LookupStream(t *testing.T) {
	t.Run("lookup stream succeeds", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			...
			rtFn := func(req *stdhttp.Request) (*stdhttp.Response, error) {
				data, err := os.Open("../../../../testdata/geoip.json")
				if err != nil {
					t.Fatalf("failed to open JSON response file: %s", err)
				}
